...
public class MessageManager {
    ...
    public MessageManager(DasJobs plugin) {
        this.plugin = plugin;
    }

    public void sendMessage(CommandSender sender, String message) {
        sender.sendMessage(plugin.getPrefix() + ChatUtil.colorize(message));
    }
    ...
    public void sendCenteredMessage(Player player, String message) {
        ...
        message = ChatUtil.colorize(message);
        int messagePxSize = 0;
    ...
    private enum DefaultFontInfo {
        ...
        NUM_0('0', 5), NUM_1('1', 5), NUM_2('2', 5), NUM_3('3', 5), NUM_4('4', 5), NUM_5('5', 5),
        NUM_6('6', 5), NUM_7('7', 5), NUM_8('8', 5), NUM_9('9', 5),

        EXCLAMATION_POINT('!', 1), AT_SIGN('@', 6), NUMBER_SIGN('#', 5), DOLLAR_SIGN('$', 5),
        PERCENT('%', 5), CIRCUMFLEX('&', 5), ASTERISK('*', 5),
        LEFT_PARENTHESIS('(', 3), RIGHT_PARENTHESIS(')', 3), MINUS('-', 5), UNDERSCORE('_', 5),
        ...
        QUESTION_MARK('?', 5), SLASH('/', 5), BACK_SLASH('\\', 5), PIPE('|', 1), TILDE('~', 5),
        ...
        public static DefaultFontInfo getDefaultFontInfo(char character) {
            for (DefaultFontInfo dFI : DefaultFontInfo.values()) {
                if (dFI.getCharacter() == character) return dFI;
            }
